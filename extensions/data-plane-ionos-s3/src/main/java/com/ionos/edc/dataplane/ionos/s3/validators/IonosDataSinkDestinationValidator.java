...
package com.ionos.edc.dataplane.ionos.s3.validators;
...
public class IonosDataSinkDestinationValidator implements Validator<DataAddress> {

    @Override
    public ValidationResult validate(DataAddress dataAddress) {
        var violations = Stream.of(EDC_DATA_ADDRESS_TYPE_PROPERTY, EDC_DATA_ADDRESS_KEY_NAME, EDC_DATA_ADDRESS_SECRET, BUCKET_NAME, ENDPOINT, MAX_FILES)
                .map(it -> {
